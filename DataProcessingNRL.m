...
% %Remove this when experience is done
% BikeFacObs=dataObs{14};
% uniqueobs=unique(Obs);
% uniqueobs(1)=[];
% linkflowobs=zeros(length(uniqueobs),2);
% linkflowobs(:,1)=uniqueobs;
% for i=1:length(uniqueobs)
%     linkflowobs(i,2)=sum(sum(Obs==uniqueobs(i)));
% end
% [Y,I]=sort(linkflowobs(:,2));
% linkflowobssorted=linkflowobs(I,:);
% %at this point we run loadDataNRL
% indices=zeros(length(uniqueobs),1);
% for i=1:length(uniqueobs)
%     if linkflowobssorted(i)>21188
%         indices(i)=mappingLinks(linkflowobssorted(i));
%     else
%         indices(i)=linkflowobssorted(i);
%     end
% end
% testtruc=TableAttributes(indices,17);
% TableObsFacilities=table(linkflowobssorted(:,1),linkflowobssorted(:,2),'VariableNames',{'LinkIDs','ObservedxTimes'});
% TableObsFac=[TableObsFacilities TableAttributes(indices,15) TableAttributes(indices,17)];
% rmupcount=0;
% bikelanecount=0;
% bikeblvd=0;
% for i = 1:32052
%     if strcmp(BikeFacObs{i},'rmup');
%         rmupcount=rmupcount+1;
%     elseif strcmp(BikeFacObs{i},'BikeLanes');
%         bikelanecount=bikelanecount+1;
%     elseif strcmp(BikeFacObs{i},'blvd');
%         bikeblvd=bikeblvd+1;
%     end
% end
...
%% Save data to filesqstat

[i,j,val] = find(Obs);
data_dump = [i,j,val];
save('TripsObservationsNRLAllObsConnectedNetwork.txt','data_dump','-ascii');

[i,j,val] = find(Incidence);
data_dump = [i,j,val];
save('IncidenceNRLAllObsConnectedNetwork.txt','data_dump','-ascii');

dlmwrite('fullLinksAttsNRLAllObsConnectedNetwork.txt',dataFullLinksIDs);

[i,j,val] = find(MappingDestDummy);
data_dump = [i,j,val];
save('mappingDestDummyAllObsNRL.txt','data_dump','-ascii');
