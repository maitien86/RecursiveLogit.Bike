...
file_linkIncidence = '/home/zimmae/Documents/RL2.6.DeC.GLS/Input/Incidence.txt'; %matrix of all possible state transitions (1 if transition between 2 states is possible, 0 else)
file_observations = '/home/zimmae/Documents/RL2.6.DeC.GLS/Input/TripsObservations.txt';
file_linkAttributes = '/home/zimmae/Documents/RL2.6.DeC.GLS/Input/fullLinksAtts.txt';
...
% To remove observations that use restricted directions 
% when not using 2WaysNetwork
FalseObs=FindFalseObs();
[indexFalseObsRow,indexFalseObsCol]=find(FalseObs);
indexFalseObs=unique(indexFalseObsRow);
Obs(indexFalseObs,:)=[]; % to remove obs that do forbidden state transitions (e.g. using links in restricted direction)
...
stopWay2(zeroindex)=[]; %removes links that don't have a "Way2" because they have a restriction in FT or TF direction
