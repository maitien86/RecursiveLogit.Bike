...
[row,col]=find(ODmat);
NbOD=length(row);
Flowr20=zeros(nbAbsorbLinks,1);
Flowr10=zeros(nbAbsorbLinks,1);
ProbaObs=zeros(NbOD*r,1);
start=tic;
for n = 1:NbOD
    %n
    dest = col(n);
    ...
    for i = 1: nbDemand
        ...
        while k ~= dummy
            ...
            for j = 1: nbInd
                ...
                totUtil=totUtil + u(k,ind(j)) + V(ind(j));
                if bestUtilities < utility
                    bestInd = ind(j);
                    bestUtilities = utility;
                end
            end
            probaLink=(u(k,bestInd)+V(bestInd))/totUtil;
            probaPath=probaPath*probaLink;
            if bestInd ~= dummy
                Obs((n-1)*nbDemand + i, t) = bestInd;
                ProbaObs((n-1)*nbDemand + i) = probaPath;
                t = t + 1;
            end
            k = bestInd;
        end
    ...
    sumProbar20=sum(ProbaObs((n-1)*nbDemand+1:(n-1)*nbDemand+r));
    sumProbar10=sum(ProbaObs((n-1)*nbDemand+1:(n-1)*nbDemand+r/2));
    for i=1:r
        for j=1:sum(Obs((n-1)*nbDemand + i,:)>0)
            Flowr20(Obs((n-1)*nbDemand + i, j))=Flowr20(Obs((n-1)*nbDemand + i, j))+ODmat(orig,dest)*ProbaObs((n-1)*nbDemand + i)/sumProbar20;
            if i<=r/2
                Flowr10(Obs((n-1)*nbDemand + i, j))=Flowr10(Obs((n-1)*nbDemand + i, j))+ODmat(orig,dest)*ProbaObs((n-1)*nbDemand + i)/sumProbar10;
            end
        end
    end
end
...
save('Flowr20.mat','Flowr20');
save('Flowr10.mat','Flowr10');
% save('Obs.mat','Obs')